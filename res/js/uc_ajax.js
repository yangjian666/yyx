...
function Ajax(recvType, waitId) {
	...
	aj.waitId = waitId ? $get(waitId) : null;//public
	...
	aj.setWaitId = function(waitid) {
		aj.waitId = typeof waitid == 'object' ? waitid : $get(waitid);
	}
	...
	aj.showLoading = function() {
		if(aj.waitId && (aj.XMLHttpRequest.readyState != 4 || aj.XMLHttpRequest.status != 200)) {
			...
			aj.waitId.innerHTML = '<span><img src="/themes/mall/pvc123/default/images/loading.gif"> ' + aj.loading + '</span>';
		}
	}
...
function appendscript(src, text, reload, charset) {
	...
	try {
		...
		$get('append_parent').appendChild(scriptNode);
	} catch(e) {}
}
...
function ajaxpost(formid, func, timeout) {
	...
	if(ajaxframe == null) {
		...
		$get('append_parent').appendChild(ajaxframe);
	}
	$get(formid).target = ajaxframeid;
	$get(formid).action = $get(formid).action + '&inajax=1';
...
function ajaxpost_load() {
	...
	var s = $($get('ajaxframe').contentWindow.document.body).html();
	...
	if(!evaled && $get(formstatus)) {
		$get(formstatus).style.display = '';		
		ajaxinnerhtml($get(formstatus), s);
		evalscript(s);
	}
...
function ajaxmenu(e, ctrlid, isbox, timeout, func) {
	...
	if(!div) {
		...
		$get('append_parent').appendChild(div);
	}

	var x = new Ajax();
	var href = !isUndefined($get(ctrlid).href) ? $get(ctrlid).href : $get(ctrlid).attributes['href'].value;
	x.div = div;
	x.etype = e.type;
...
function showloading(display, wating) {
	...
	$get('ajaxwaitid').innerHTML = wating;
	$get('ajaxwaitid').style.display = display;
}
...
function ajaxinnerhtml(showid, s) {
	if(showid.tagName != 'TBODY') {
		...
		$get('append_parent').appendChild(div1);
