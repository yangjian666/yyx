...
var GuessPlayHelper = {
  ...
  checkWealth: function (playWayId) {
    ...
    var playWealth = notNanParseFloat(playWealthInput.val());
    var max = notNanParseFloat(playWealthInput.attr('max'));
    if (playWealth < 0) {
      playWealth = 0;
    ...
    oddsTip.html((oods * playWealth).toFixed(8));
  },
  addPlay: function (playWayId) {
    ...
    playData.setWealth(notNanParseFloat($('#play_wealth_' + playWayId).val()));
    if (!playData.isPlay()) {
      // 删除总计部分
