...
function PlayWayData() {
	...
	PlayWayData.prototype.setBettingLowerLimit = function(bettingLowerLimit) {
	    this.bettingLowerLimit = notNanParseFloat(bettingLowerLimit);
	};
	...
	PlayWayData.prototype.setBettingUpperLimit = function(bettingUpperLimit) {
	    this.bettingUpperLimit = notNanParseFloat(bettingUpperLimit);
	};
...
var GuessAddHelper = {
    ...
    setWealthType : function(wealthType) {
      if(!this.wealthIsEnough(wealthType)){
        ...
        }else if(wealthType == this.WEATH_TYPE_LTC){
          alert('你当前的莱特币不足以支付所有玩法所需的托管积分');
        }else if(wealthType == this.WEATH_TYPE_BTC){
          alert('你当前的比特币不足以支付所有玩法所需的托管积分');
        }else if(wealthType == this.WEATH_TYPE_DOGE){
          alert('你当前的狗币不足以支付所有玩法所需的托管积分');
        }
        return false;
      }
  ...
  setUserLtc : function(userMoney) {
    this.userLtc = userMoney;
  },
  getUserLtc : function() {
    return this.userLtc;
  },
  setUserBtc : function(userMoney) {
    this.userBtc = userMoney;
  },
  getUserBtc : function() {
    return this.userBtc;
  },
  setUserDoge : function(userMoney) {
    this.userDoge = userMoney;
  },
  getUserDoge : function() {
    return this.userDoge;
  },
  ...
  wealthTypeIsLtc : function() {
    return this.wealthType == this.WEATH_TYPE_LTC;
  },
  wealthTypeIsBtc : function() {
    return this.wealthType == this.WEATH_TYPE_BTC;
  },
  wealthTypeIsDoge : function() {
    return this.wealthType == this.WEATH_TYPE_DOGE;
  },
    // 添加一个玩法数据
	...
	if(this.wealthTypeIsIntegral()){
      ...
      if(this.wealthTypeIsLtc()){
        leaveWealth = this.userLtc - this.needKeepWealth() + oldOeedKeepWealth;
      }
      if(this.wealthTypeIsBtc()){
        leaveWealth = this.userBtc - this.needKeepWealth() + oldOeedKeepWealth;
      }
      if(this.wealthTypeIsDoge()){
        leaveWealth = this.userDoge - this.needKeepWealth() + oldOeedKeepWealth;
      }
	...
	if(typeof (wealthType) != 'undefined'){
        ...
        return this.userLtc >= needKeepWealth;
      }else if(wealthType == this.WEATH_TYPE_BTC){
        return this.userBtc >= needKeepWealth;
      }else if(wealthType == this.WEATH_TYPE_DOGE){
        return this.userDoge >= needKeepWealth;
      }
	}else{
	    if(this.wealthTypeIsMoney()){
		return this.userMoney >= needKeepWealth;
