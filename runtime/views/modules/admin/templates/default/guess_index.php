		                                 ...
		                                 &nbsp;
		                               	...
		                               	<select name="wealth_type">
                                            ...
                                            <option value="2" <?php if($wealth_type == 2){ ?>selected<?php } ?>>积分</option>
		                               		<option value="6" <?php if($wealth_type == 3){ ?>selected<?php } ?>>自定义</option>
		                               	</select>										
										&nbsp;
										竞猜状态:
		                               	<select name="status">
					...
					<tbody>
						...
						<tr>
								...
								<td align="left">
									...
									<?php }elseif($item['status'] == 3){ ?>已结算
									<?php }elseif($item['status'] == 4){ ?>已关闭
                                    <?php }elseif($item['status'] == 5){ ?>已解散
									<?php }else{ ?>不详<?php } ?>
								</td>
								...
								<td align="left">
                                    ...
                                    <a href="<?php echo url("/$currentModule/$currentAction/edit/?id=$item[id]") ?>">修改截止时间</a>&nbsp;
                                    <?php }elseif($item['status'] == 1){ ?>
                                    <a href="<?php echo url("/$currentModule/$currentAction/edit/?id=$item[id]") ?>">修改截止时间</a>&nbsp;
									<?php }elseif($item['status'] == 2){ ?>
									<a href="<?php echo url("/$currentModule/$currentAction/rudge/?id=$item[id]") ?>">结果判定</a>&nbsp;
                                    <a href="<?php echo url("/$currentModule/$currentAction/dimiss/?id=$item[id]") ?>">解散</a>&nbsp;
									<?php } ?>
								</td>
						</tr>
						<?php } ?>
