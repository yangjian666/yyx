<script type="text/javascript" src="http://www.yyx-test.com//res/js/My97DatePicker/WdatePicker.js"></script>
<script type="text/javascript">
<!--
GuessAddHelper.reset();
GuessAddHelper.setUserMoney(<?php echo $user->getAvailableMoney(); ?>);
GuessAddHelper.setUserIntegral(<?php echo $user->getAvailableIntegral(); ?>);
GuessAddHelper.setUserLtc(<?php echo $user->getAvailableLtc(); ?>);
GuessAddHelper.setUserBtc(<?php echo $user->getAvailableBtc(); ?>);
GuessAddHelper.setUserDoge(<?php echo $user->getAvailableDoge(); ?>);
//-->
</script>
<dt>竞猜标题：</dt>
<dd class="c9">
...
<dd>
	<label class="radio inline">
		<input type="radio" name="wealth_type" value="<?php echo Guess::WEALTH_TYPE_MONEY ?>" checked="checked" onclick="GuessAddHelper.setWealthType(this.value)"/> 金币投注
	</label>
    <label class="radio inline">
        <input type="radio" name="wealth_type" value="<?php echo Guess::WEALTH_TYPE_BTC ?>" checked="checked" onclick="GuessAddHelper.setWealthType(this.value)"/> BTC投注
    </label>
    <label class="radio inline">
        <input type="radio" name="wealth_type" value="<?php echo Guess::WEALTH_TYPE_LTC ?>" checked="checked" onclick="GuessAddHelper.setWealthType(this.value)"/> LTC投注
    </label>
    <label class="radio inline">
        <input type="radio" name="wealth_type" value="<?php echo Guess::WEALTH_TYPE_DOGE ?>" checked="checked" onclick="GuessAddHelper.setWealthType(this.value)"/> DOGE投注
    </label>
	<label class="radio inline">
