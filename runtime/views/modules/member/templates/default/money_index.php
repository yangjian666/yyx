	...
	<div class="setting_body">
        ...
        <div class="tab-content setting_list">
			<div class="tab-pane active">
				<div class="mymoney">
					<p class="mymoneyinfo">当前可用<?php echo $user->getWealthTypeName($_GET['wealth_type']); ?>为：<span><?php $funcName = $user->getWealthTypeFuncName($_GET['wealth_type']) ?><?php echo $user->$funcName(); ?></span>, 冻结金币为：<span><?php $funcName = $user->getFreeTypeFuncName($_GET['wealth_type']) ?><?php echo $user->$funcName(); ?></span></p>
                    <a class="btn btn-success paybtn"  href="/member/recharge/index/?wealth_type=<?php echo $_GET['wealth_type']; ?>" id="money_recharge" onclick="ajaxmenu(event, this.id, 1)">充 值</a>&nbsp; &nbsp;
                    <a class="btn btn-success paybtn"  href="/member/money/handsel/?wealth_type=<?php echo $_GET['wealth_type']; ?>" id="money_handsel" onclick="ajaxmenu(event, this.id, 1)">转 赠</a>&nbsp; &nbsp;
                    <a class="btn btn-success paybtn"  href="/member/withdraw/handsel/?wealth_type=<?php echo $_GET['wealth_type']; ?>" id="money_withdraw" onclick="ajaxmenu(event, this.id, 1)">提现</a>
                </div>
