...
class GuessAction extends AbstractAdminAction{
     ...
     * @param HttpRequest $request
     */
    public function editDeadline(HttpRequest $request){
        $id = $request->getParameter('id');
        $deadline = intval($request->getParameter('deadline'));
        if(empty($id) || empty($deadline)){
            AjaxResult::ajaxResult(1, '没有指定竞猜');
        }
        ...
        if($guess->getStatus() != Guess::STATUS_WAITING_CKECK && $guess->getStatus() != Guess::STATUS_NORMAL){
            AjaxResult::ajaxResult(1, '竞猜状态不对');
        }

        //校验竞猜时间合法性
        if($deadline < intval($guess->getPlayDeadline())){
            AjaxResult::ajaxResult(1, '截止时间不能早于开始时间');
        }

        $guessPoint = $guessService->getGuessPoint($guess->getGuessPointId());
        if(!$guessPoint){
            AjaxResult::ajaxResult(1, '竞猜点不存在');
        }
        if($deadline > intval($guessPoint->getDlayDeadline())){
            AjaxResult::ajaxResult('1', '截止时间不能晚于竞猜点截止时间');
        }

        //改变状态
        $success = MD('Guess')->update(array('play_deadline' =>  $deadline), $guess->getId());
        if (!$success) {
            AjaxResult::ajaxResult('1', '修改失败');
        }
