...
class UserService extends TransationSupport implements IUserService{
	...
	*/
	...
	public function register(User $user){
        ...
        //发送注册通知 并根据赠送配置赠送金币
        $registerMoney = floatval(R::getConfig()->getConfig('wealth_register'));
        if($registerIntegral){
            //赠送
            $io = array(
                'from_user_id' => 0,
                ...
                'wealth' => $registerMoney,
                'from_balance' => $registerMoney
            );
            $this->money($io);
            //通知
            $notice = "欢迎你注册成为" . R::getConfig()->getConfig('site_name') . "会员，系统赠送你{$registerMoney}金币";
            MemberServiceFactory::getNoticeService()->notice($notice, $userId);
        }
        //发送注册通知 并根据赠送配置赠送BTC
        $registerMoney = floatval(R::getConfig()->getConfig('wealth_btc_register'));
        if($registerIntegral){
            //赠送
            $io = array(
                'from_user_id' => 0,
                'to_user_id' => $userId,
                'to_title' => "注册赠送",
                'wealth_type' => Io::WEALTH_TYPE_BTC,
                'wealth' => $registerMoney,
                'from_balance' => $registerMoney
            );
            $this->money($io);
            //通知
            $notice = "欢迎你注册成为" . R::getConfig()->getConfig('site_name') . "会员，系统赠送你{$registerMoney}BTC";
            MemberServiceFactory::getNoticeService()->notice($notice, $userId);
        }
        //发送注册通知 并根据赠送配置赠送LTC
        $registerMoney = floatval(R::getConfig()->getConfig('wealth_ltc_register'));
        if($registerIntegral){
            //赠送
            $io = array(
                'from_user_id' => 0,
                'to_user_id' => $userId,
                'to_title' => "注册赠送",
                'wealth_type' => Io::WEALTH_TYPE_LTC,
                'wealth' => $registerMoney,
                'from_balance' => $registerMoney
            );
            $this->money($io);
            //通知
            $notice = "欢迎你注册成为" . R::getConfig()->getConfig('site_name') . "会员，系统赠送你{$registerMoney}LTC";
            MemberServiceFactory::getNoticeService()->notice($notice, $userId);
        }
        //发送注册通知 并根据赠送配置赠送金币DOGE
        $registerMoney = floatval(R::getConfig()->getConfig('wealth_doge_register'));
        if($registerIntegral){
            //赠送
            $io = array(
                'from_user_id' => 0,
                'to_user_id' => $userId,
                'to_title' => "注册赠送",
                'wealth_type' => Io::WEALTH_TYPE_DOGE,
                'wealth' => $registerMoney,
                'from_balance' => $registerMoney
            );
            $this->money($io);
            //通知
            $notice = "欢迎你注册成为" . R::getConfig()->getConfig('site_name') . "会员，系统赠送你{$registerMoney}DOGE";
            MemberServiceFactory::getNoticeService()->notice($notice, $userId);
        }
		try{
			foreach($this->listeners as $listener){
	...
	public function money($io, $freezeCode = 0){
		...
		if($freezeCode){
			if($freezeCode > 0){
				// 解冻
				$userId = $io['to_user_id'];
                switch($io['wealth_type']){
                  case Io::WEALTH_TYPE_MONEY:
                      $update['available_money'] = "available_money + {$io['wealth']}";
                      $update['freeze_money'] = "freeze_money - {$io['wealth']}";
                      break;
                  case Io::WEALTH_TYPE_LTC:
                      $update['available_ltc'] = "available_ltc + {$io['wealth']}";
                      $update['freeze_ltc'] = "freeze_ltc - {$io['wealth']}";
                      break;
                  case Io::WEALTH_TYPE_BTC:
                      $update['available_btc'] = "available_btc + {$io['wealth']}";
                      $update['freeze_btc'] = "freeze_btc - {$io['wealth']}";
                      break;
                  case Io::WEALTH_TYPE_DOGE:
                      $update['available_doge'] = "available_doge + {$io['wealth']}";
                      $update['freeze_doge'] = "freeze_doge - {$io['wealth']}";
                      break;
                }
			}else{
				// 冻结
