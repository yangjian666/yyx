...
class Guess extends DynamicModelTransformSupport{
     ...
     * return boolean
     */
    public function getWealthTypeFuncName(){
        $index = $this->wealthType;
        $map = array('',
            'getAvailableMoney',
     ...
     * return boolean
     */
    public function getWealthTypeWealthTypeName(){
        $index = $this->wealthType;
        $map = array('',
            'available_money',
            'available_integral',
     ...
     * return boolean
     */
    public function getWealthTypeFreezWealthTypeName(){
        $index = $this->wealthType;
        $map = array('',
            'freeze_money',
            'freeze_integral',
     ...
     * return boolean
     */
    public function wealthTypeIsWealth(){
        $type = $this->wealthType;
        return ($type == Guess::WEALTH_TYPE_MONEY ||
            $type == Guess::WEALTH_TYPE_INTEGRAL ||
            $type == Guess::WEALTH_TYPE_LTC ||
            $type == Guess::WEALTH_TYPE_BTC ||
            $type == Guess::WEALTH_TYPE_DOGE
        );
    }
	/**
	 * 是否是金币类型财富
    ...
    public function wealthTypeIsLtc(){
        return $this->wealthType == Guess::WEALTH_TYPE_LTC;
    }
    public function wealthTypeIsBtc(){
        return $this->wealthType == Guess::WEALTH_TYPE_BTC;
    }
    public function wealthTypeIsDoge(){
        return $this->wealthType == Guess::WEALTH_TYPE_DOGE;
    }
	/**
	 * 是否是积分类型财富
	 * return boolean
	...
	public function wealthIsEnough(User $user){
		...
		$leaveWealth = 0;
		if($this->wealthTypeIsWealth()){
            $typeFun = $this->getWealthTypeFuncName();
            if($typeFun){
                $leaveWealth = $user->$typeFun();
            }
	...
	public function getWealthTypeName(){
		if($this->wealthTypeIsMoney()){
			return "金币";
		}elseif($this->wealthTypeIsLtc()){
            return  "莱特币";
        }elseif($this->wealthTypeIsBtc()){
            return  "比特币";
        }elseif($this->wealthTypeIsDoge()){
            return  "狗币";
