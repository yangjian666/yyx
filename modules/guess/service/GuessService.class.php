			...
			if($keepWealth){
				...
				if($guess->wealthTypeIsWealth()){
                    ...
                    $io = array(
                        ...
                        'wealth' => $keepWealth,
                        'from_balance' => $user->$typeFunc() - $keepWealth
                    );
		...
		try{
			...
			foreach($guesses as $guess){
				...
				foreach($plays as $play){
					...
					if($guess->wealthTypeIsWealth()){
						$io['wealth_type'] = $guess->getWealthType();
                        $typeFunc = $guess->getWealthTypeFuncName();
						if($playWinWealth >= 0){
							//赢
						...
						$io['to_balance'] = $playUser->$typeFunc() + $io['wealth'];
                        ...
                        $wealthName = $guess->getWealthTypeWealthTypeName();
                        $freezWealthName = $guess->getWealthTypeFreezWealthTypeName();
                        $update[$wealthName] = $wealthName . " + {$io['wealth']}";
                        $update[$freezWealthName] = $freezWealthName . " + {$io['wealth']}";
				...
				if($guess->wealthTypeIsWealth()){
					...
					$io['to_balance'] = $guessUser->$typeFunc() + $io['wealth'];
