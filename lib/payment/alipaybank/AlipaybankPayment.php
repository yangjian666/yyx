...
require_once(dirname(__FILE__) . DIRECTORY_SEPARATOR . "../alipay/lib/alipay_service.class.php");
require_once(dirname(__FILE__) . DIRECTORY_SEPARATOR . "../alipay/lib/alipay_notify.class.php");
class AlipaybankPayment extends Payment{
	...
	public function pay($requestParameter) {
		...
		if(empty($partner)){
			throw new PaymentException("合作者信息不能为空", PaymentException::EX_CODE_PAYMENT_PAY_ERROR, $this->getPaymentType());
		}
		//根据合作者信息创建支付宝合作者信息数组
		$aliapyConfig = array(
		...
		if(empty($requestParameter['sign_type'])){
			$requestParameter['sing_type'] = $paymentConfig->getSign();
		}
		$aliapyConfig['sign_type'] = $requestParameter['sign_type'];
		if(empty($requestParameter['input_charset'])){
			$requestParameter['input_charset'] = $paymentConfig->getCharset();
		}
		...
		if(empty($requestParameter['defaultbank'])){
			//默认为中国工商银行
			$requestParameter['defaultbank'] = 'icbcb2c';
		}
		//创建支付宝服务类
